  ...
  const topBarHTML = `
    <div class="top-bar">
      ...
      <div class="theme-toggle">
        <button id="langToggle" class="glass-btn small" style="margin-right:8px;">العربية</button>
...
function handleReportSubmit(event) {
  ...
  pushAudit('report', 'Report submitted', { id: complaint.id, category: complaint.category, location: complaint.location });
...
function handleRegister(e) {
  ...
  if (!idCardInput || !idCardInput.files || !idCardInput.files[0]) {
    showErr("Please upload your ID card image."); return;
  }
  ...
  reader.onload = function(ev) {
    const idCardImage = ev.target.result;
  ...
  reader.readAsDataURL(idCardInput.files[0]);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (registerBtnEl && registerFormEl) {
    registerBtnEl.addEventListener('click', (e) => {
      e.preventDefault();
      if (registerFormEl.checkValidity && !registerFormEl.checkValidity()) {
        registerFormEl.reportValidity && registerFormEl.reportValidity();
        return;
      }
      handleRegister(e);
    });
  ...
  document.getElementById('quickReportBtn')?.addEventListener('click', () => {
    ...
    input.addEventListener('change', function() {
      ...
      if (file) {
        ...
        reader.onload = function(ev) {
          window.lastCapturedImageData = ev.target.result;
